...
Run this script from the directory containing the mogwai, gitian-builder, gitian.sigs, and mogwai-detached-sigs.
...
-u|--url	Specify the URL of the repository. Default is https://randallpeltzer@bitbucket.org/mogwaicoin/mogwai.git
...
then
  ...
  pushd ./gitian-builder
	...
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../mogwai/contrib/gitian-descriptors/gitian-win-signer.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-win-signed --destination ../gitian.sigs/ ../mogwai/contrib/gitian-descriptors/gitian-win-signer.yml
	    mv build/out/mogwai-*win64-setup.exe ../mogwai/bin/${VERSION}
	    mv build/out/mogwai-*win32-setup.exe ../mogwai/bin/${VERSION}
	fi
	# Sign Mac OSX
	if [[ $osx = true ]]
	then
	    ...
	    echo ""
	    ./bin/gbuild -i --commit signature=${COMMIT} ../mogwai/contrib/gitian-descriptors/gitian-osx-signer.yml
	    ./bin/gsign -p $signProg --signer $SIGNER --release ${VERSION}-osx-signed --destination ../gitian.sigs/ ../mogwai/contrib/gitian-descriptors/gitian-osx-signer.yml
	    mv build/out/mogwai-osx-signed.dmg ../mogwai/bin/${VERSION}/mogwai-${VERSION}-osx.dmg
	fi
	popd
