...
UniValue getinfo(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total mogwai balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized mogwai balance of the wallet\n"
...
UniValue debug(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "mogwai|privatesend|instantsend|masternode|spork|keepass|mnpayments|gobject )\n"
            "Change debug category on the fly. Specify single category or use comma to specify many.\n"
            "\nExamples:\n"
            + HelpExampleCli("debug", "mogwai")
            + HelpExampleRpc("debug", "mogwai,net")
        );
...
UniValue pubkey2address(const UniValue& params, bool fHelp)
{
	...
	if (isValid)
	{
		bool pubKeyIsFullyValid = pubKey.IsFullyValid();
		ret.push_back(Pair("pubKeyIsFullyValid", pubKeyIsFullyValid));
...
UniValue mirroraddress(const UniValue& params, bool fHelp)
{
	if (fHelp || params.size() != 1)
		throw runtime_error(
			"mirroraddress \"mogwaiaddress\"\n"
			"\nReturn the mirror address of a given mogwai address.\n"
			"\nArguments:\n"
			"1. \"mogwaiaddress\"     (string, required) The mogwai address to mirror\n"
			"\nResult:\n"
			"{\n"
			"  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
			"  \"address\" : \"mogwaiaddress\", (string) The mogwai address validated\n"
...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"mogwaiaddress\"\n"
            "\nReturn information about the given mogwai address.\n"
            "\nArguments:\n"
            "1. \"mogwaiaddress\"     (string, required) The mogwai address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"mogwaiaddress\", (string) The mogwai address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Mogwai address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are mogwai addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) mogwai address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"mogwaiaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"mogwaiaddress\"  (string, required) The mogwai address to use for the signature.\n"
