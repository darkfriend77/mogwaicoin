...
CBlockTemplate* CreateNewBlock(const CChainParams& chainparams, const CScript& scriptPubKeyIn)
{
...

    {
        ...
        txNew.vin[0].scriptSig = CScript() << nHeight << OP_0; // randall: check ...
        ...
        LogPrintf("--> BlockPayments: nBlockHeight %d blockReward %lld txoutMasternode %s txNew %s",
...
// ***TODO*** ScanHash is not yet used in Mogwai
//
...
{
    LogPrintf("MogwaiMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mogwai-miner");
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("MogwaiMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            CBlock *pblock = &pblocktemplate->block;
            IncrementExtraNonce(pblock, pindexPrev, nExtraNonce);

            LogPrintf("MogwaiMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("MogwaiMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("MogwaiMiner -- runtime error: %s\n", e.what());
        return;
    }
}
