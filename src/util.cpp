...
const char * const BITCOIN_CONF_FILENAME = "mogwai.conf";
const char * const BITCOIN_PID_FILENAME = "mogwaid.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "mogwai" is a composite category enabling all Mogwai-related debug output
            if(ptrCategory->count(string("mogwai"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MogwaiCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MogwaiCore
    // Mac: ~/Library/Application Support/MogwaiCore
    // Unix: ~/.mogwaicore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "MogwaiCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MogwaiCore";
#else
    // Unix
    return pathRet / ".mogwaicore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty mogwai.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override mogwai.conf
